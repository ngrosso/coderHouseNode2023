...
export const sendMail = async (targetMail: string, ticket: any, purchased: any[], stillInCart: any[], paymentUrl: string): Promise<any> => {
  ...
  <html>
    ...
    <p>Order details:</p>
    <ul>
      ${purchased.forEach((product: any) => `<li>${product.quantity}x ${product.product.title} --- Unit Price $${product.product.price} = $${product.product.price * product.quantity}</li>`)}
    </ul>
    <p>Total: $${ticket.amount}</p>
    <br>
    ${stillInCart.length > 0 ? `<h3>Some products are out of stock</h3>` : ''}
    <ul>
    ${stillInCart.forEach((product: any) => `<li>${product.quantity}x ${product.product.title} --- Unit Price $${product.product.price} = $${product.product.price * product.quantity}</li>`)}
    </ul>
  </html>`;
...
export const forgotPasswordMailer = async (targetMail: string, token: string): Promise<any> => {
